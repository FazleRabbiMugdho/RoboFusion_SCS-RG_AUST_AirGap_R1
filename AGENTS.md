...
|-------|------|-----------|
| FastAPI routers | `backend/app/routers/` | One file per resource: `zones.py`, `readings.py`, `incidents.py`, `auth.py`, `admin.py` |
...
- **Prompt 30 (`feat/incident-history-view` / PR #45):** Added `primary_hazard_type` DB migration & persistence in `record_state_transition`, created `GET /api/v1/incidents` endpoint with keyset pagination & date-range validation, built `IncidentHistoryView.tsx` with virtualized table (`@tanstack/react-virtual`), filter bar, CSV export, and 4 UI states.
- **Prompt 31 (`feat/system-health-view` / PR #46):** Built admin-only system health router (`GET /api/v1/admin/zones/health`, `POST /api/v1/admin/zones/{zone_id}/override`) gated by `RequireRole("ADMIN")` with audit logging, extracted shared `zoneOffline.ts` helper, and created `SystemHealthView.tsx` with connectivity table, online/offline status, and manual actuator override controls ("Test Alert" & "Silence").
